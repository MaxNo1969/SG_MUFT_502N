...
CSG TSG::GetSG()
{
	...
	try{
	//составим список частот текущего типоразмера и значени€ порогов их эталонов
	...
	//определим наиболее частую √ѕ по всем частотам
	int maximal = 0;
	int element = 0;
...
CSG TSG::Get1FrecSG()
{
	...
	if (EDatas.Etalons.size()==0) {
		...
		ret.group = "’’’";
		ret.color = clWhite;
		 TExtFunction::ShowBigModalMessage("Ќе найдены значени€ образцов!", clRed);
		return (ret);
	}else{
	  //
    }
		...
		case 1:
		{
			...
			for(int j = 0; j < EDatas.Etalons.size(); j++)
			{
				if(TSFreqs.Frequency[0] == EDatas.Etalons[j].Frequency)
				{
					...
					//определим наименьшее и наибольшее отклонени€ и посчитаем его эталон как √ѕ дл€ этой частоты
